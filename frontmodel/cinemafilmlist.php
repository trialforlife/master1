...
if (mysql_affected_rows() > 0) {
	while($row = mysql_fetch_array($dbresult))
	{
		array_push($result["films"],array(
            ...
            '<span class="h2">'.$row["f_name"]."</span><span class='time'>".$row["f_time"]."</span><img style=\"width:100px; float:right ; height:50px;\" src=http://now/".$row["f_image"]."><pf>".$row["f_content"]."</pf>",
			));
