...
class Play extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('c_id, f_published', 'numerical', 'integerOnly'=>true),
			array('f_name, f_price', 'length', 'max'=>1000),
			array('f_time', 'length', 'max'=>10000),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('f_id, c_id, f_name, f_time, f_price, f_content, f_image, f_published', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('f_published',$this->f_published);
