...
if (mysql_affected_rows() > 0) {
	while($row = mysql_fetch_array($dbresult))
	{
		array_push($result["films"],array(
			...
			'<div class="item-element delivery"><span class="h2">'.$row["rs_name"]."</span><span class='image_mask'><img style=\"width:100px; float:left ; height:50px;\" src=http://now-yakutsk.stairwaysoft.net/mobile/img/".$row["rs_image"]."></span>".$row["rs_time"]."</br><small>".$row["rs_content"]."</small></br>".$row["rs_price"]."</div>",
			));
