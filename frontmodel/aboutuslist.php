...
if (mysql_affected_rows() > 0) {
	while($row = mysql_fetch_array($dbresult))
	{
		array_push($result["cinema"],array(
			...
			"list"=>'<div class="container about"><span class="h2">'.addslashes((string)$row["com_quote"]).'</span><p>'.addslashes((string)$row["com_description"]).'</p><div class="ab-navigtaion"><a href="tel:'.$row["com_phone"].'" class="button">'.$row["com_phone"].'</a><a href="http://'.$row["com_site"].'"class="button">'.$row["com_site"].'</a> <a href="mailto:Now%20Yakutsk%20&lt;' .$row["com_email"].'"gt; class="button no_shadow">Отправить сообщение</a> </div></div>',
