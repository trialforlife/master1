...
if (mysql_affected_rows() > 0) {
	while($row = mysql_fetch_array($dbresult))
	{
		array_push($result["cinema"],array(
			...
			"list"=>'<div class="container about">'.addslashes((string)$row["com_quote"]).'<br>'."<img src=http://now/".addslashes((string)$row["com_logo"])."><br>".addslashes((string)$row["com_description"])."<br>".$row["com_phone"].$row["com_site"],
