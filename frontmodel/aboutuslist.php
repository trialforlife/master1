...
if (mysql_affected_rows() > 0) {
	while($row = mysql_fetch_array($dbresult))
	{
		array_push($result["cinema"],array(
			...
			"list"=>'<div class="container about"><span class="h2">'.addslashes((string)$row["com_quote"]).'</span><p>'.addslashes((string)$row["com_description"]).'</p><div class="ab-navigtaion"><a href="tel:123456"><span class="button">'.$row["com_phone"].'</span></a><span class="button">'.$row["com_site"].'</span><span class="button no_shadow">Отправить сообщение</span></div></div>',
