...
$query = 'select * from beautyandhealth where  bh_published="1"';//left join `cinema_banner` on (cinema.c_id=cinema_banner.c_id)';
...
if (mysql_affected_rows() > 0) {
	while($row = mysql_fetch_array($dbresult))
	{
		array_push($result["cinema"],array(
            ...
            "list" => '<div class="nav-element1"><span class="txt">' . addslashes((string)$row["bh_name"]) . '</span><span class="s_arrow"></span><span class="location">' . addslashes((string)$row["bh_adress"]) . '</span><span class="img_box"><img class="list_image" src=http://now-yakutsk.stairwaysoft.net/image' . $row["bh_image"] . '></span></div>',
